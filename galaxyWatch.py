...
class PlotType(Enum):
    PULSE = EntryType.PULSE
    ACCELERATION = EntryType.ACCELERATION
    AIRPRESSURE = EntryType.AIRPRESSURE
    STEPS = EntryType.STEPS
    BATTERY = 2


class DataEntry:
    def __init__(self, time: datetime, battery: int, entry_type: EntryType, sensor_data: int or [int, int, int]):
        self.time: datetime = time
        self.battery: int = battery
        self.type: EntryType = entry_type
...
def get_sensor_data():
    ...
    for file in os.listdir(directory):
        ...
        for line in open('sensorData/' + os.fsdecode(file), 'r').readlines():
            ...
            sensor_id = EntryType(int(data[0]))
            if sensor_id == EntryType.OK:
                continue
            timestamp = datetime.datetime.utcfromtimestamp(int(data[1]) / 1000)
            battery = int(float(data[5]))
            sensor_data = int(float(data[2]))
            if sensor_id == EntryType.PULSE and sensor_data == 0:
                sensor_data = None
            if sensor_id == EntryType.ACCELERATION:
